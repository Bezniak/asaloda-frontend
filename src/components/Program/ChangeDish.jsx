...
const ChangeDish = ({isAdditionalMenuVisible, setIsAdditionalMenuVisible, dish}) => {
    ...
    return (
        <div
            ...
            className={`fixed top-0 right-0 z-40 h-screen p-5 overflow-y-auto transition-transform ${isAdditionalMenuVisible ? 'translate-x-0' : 'translate-x-full'} bg-white w-2/6 dark:bg-gray-800`}
            tabIndex="-1"
        ...
        >
            ...
            <div
                className="mt-10 flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                <img
                    className="object-cover w-full rounded-t-lg h-96 md:h-auto md:w-48 md:rounded-none md:rounded-s-lg"
                    src={import.meta.env.VITE_UPLOAD_URL + dish?.attributes?.main_img?.data?.attributes?.url} alt=""/>
                <div className="flex flex-col justify-between p-4 leading-normal">
                    <h2>{dish?.attributes?.dish_name}</h2>
                </div>
            </div>
